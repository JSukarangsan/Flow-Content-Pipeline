...
function App() {
  ...
  // Plays Modal State
  const [isPlaySelectorOpen, setIsPlaySelectorOpen] = useState(false);
  ...
  // Listen for file import notifications
  useEffect(() => {
    ...
    window.addEventListener('flow-notification', handleNotification as EventListener);
    return () => {
      window.removeEventListener('flow-notification', handleNotification as EventListener);
    };
  }, []);
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (isPlaySelectorOpen) {
      if (e.key === 'Escape') setIsPlaySelectorOpen(false);
      return;
    }
  ...
  }, [navState, filteredPillars, filteredExecutions, selectedPillarId, selectedExecutionId, isGeneratorOpen, isSettingsOpen, isImportOpen, isPlaySelectorOpen]);
  ...
  const handleAddPillar = async () => {
    ...
    try {
      ...
      showNotification(`Created ${newPillars.length} new pillars`);
    } catch (e) {
  ...
  const handleAnalyzeThemes = async () => {
    ...
    try {
      ...
      showNotification('Themes analyzed successfully');
    } catch (e) {
      showNotification('Failed to analyze themes', 'error');
    } finally {
      setIsAiLoading(false);
      setAiMessage('');
    }
  ...
  const openPlaySelector = () => {
    if(!selectedPillarId) return;
    setIsPlaySelectorOpen(true);
  };
  ...
  const confirmGeneration = async () => {
    ...
    try {
      ...
      showNotification(`Generated ${newExecutions.length} drafts`);
    } catch (e) {
      showNotification('Failed to generate drafts', 'error');
    } finally {
      setIsAiLoading(false);
      setAiMessage('');
    }
  ...
  const handlePlayResults = async (results: Array<{ platform: Platform; content: string }>) => {
    const pillar = pillars.find(p => p.id === selectedPillarId);
    if (!pillar) return;

    setIsAiLoading(true);
    setAiMessage('Creating posts from play...');
    ...
    try {
      ...
      for (const result of results) {
        const newExecution: Execution = {
          id: `play-e-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
          pillarId: pillar.id,
      ...
      showNotification(`Created ${newExecutions.length} posts from play`);
    } catch (e) {
      showNotification('Failed to save play results', 'error');
    } finally {
      setIsAiLoading(false);
      setAiMessage('');
    }
  ...
  const handleExportPDF = async () => {
    try {
      setIsAiLoading(true);
      setAiMessage('Generating PDF...');

      const timestamp = new Date().toISOString().split('T')[0];
      await pdfExporter.savePDF(
        `content-strategy-${timestamp}.pdf`,
        pillars,
        executions,
        {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950 text-gray-200 font-sans selection:bg-accent-500/30 selection:text-white">
      ...
      {/* Notification Toast */}
      {notification && (
        <div className={`fixed top-4 right-4 z-50 p-4 rounded-lg shadow-lg max-w-md animate-fade-in-down ${
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        >
          <div className="p-4 border-b border-gray-800 flex justify-between items-center shrink-0">
              ...
              <div className="flex space-x-2">
                <button
                  onClick={openPlaySelector}
                  className="text-xs hover:text-white text-gray-500 flex items-center disabled:opacity-50 transition-colors"
           ...
           <div className="overflow-y-auto flex-1 pb-10">
              {filteredExecutions.length === 0 ? (
                <div className="p-8 text-center text-gray-700 text-sm">
                  No drafts yet. Select a pillar and generate ideas or run a play.
                </div>
              ) : (
      ...
      {/* PLAY SELECTOR MODAL */}
      {isPlaySelectorOpen && selectedPillarId && (
        <PlaySelector
          pillar={pillars.find(p => p.id === selectedPillarId)!}
          onClose={() => setIsPlaySelectorOpen(false)}
          onExecute={handlePlayResults}
          settings={settings}
        />
      )}
